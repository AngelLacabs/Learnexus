...
/* =====================
   ...
   Fetch after generation to ensure we have the latest voucher
===================== */
$voucher = null;
try {
    $stmt = $conn->prepare("
    ...
    if ($voucher) {
        error_log("Voucher found: ID=" . $voucher['voucherID'] . ", Code=" . $voucher['voucherCode'] . ", Status=" . $voucher['voucherStatus']);
    } else {
        error_log("WARNING: No voucher found for certificateID " . $certificate['certificateID']);
    }
} catch (Exception $e) {
    ...
    setTimeout(function() {
        ...
        if (modalElement) {
            try {
                ...
                voucherModal.show();
                console.log('✅ Voucher modal displayed successfully');
            } catch (error) {
                console.error('❌ Error showing voucher modal:', error);
            }
        } else {
            console.error('❌ Voucher modal element not found in DOM');
        }
    }, 1500);
});
<?php else: ?>
console.log('⚠️ No voucher found for certificate. Voucher variable is null or empty.');
<?php endif; ?>
</script>
